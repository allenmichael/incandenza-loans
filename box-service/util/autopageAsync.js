...
function automateItemPaginationWithOffset(client, manager, methodName, id, options) {
  return asyncFunc(function* () {
    let collection = [];
    options = options || {};
    options.offset = options.offset || 0;
    options.limit = options.limit || ITEM_LIMIT;
    collection = yield continuePagingWithOffset(client, manager, methodName, id, options, collection, options.offset);
    return collection;
  })();
}
...
function automateItemPaginationWithMarker(client, manager, methodName, id, options) {
  return asyncFunc(function* () {
    var collection = [];
    options = options || {};
    options.limit = options.limit || ITEM_LIMIT;
...
function automateItemPaginationWithStream(client, manager, methodName, id, options) {
  return asyncFunc(function* () {
    var collection = [];
    options = options || {};
    options.limit = options.limit || ITEM_LIMIT;
...
module.exports = {
  autoPageWithOffsetAsync: automateItemPaginationWithOffset,
  autoPageWithMarkerAsync: automateItemPaginationWithMarker,
  autoPageWithStreamAsync: automateItemPaginationWithStream
};